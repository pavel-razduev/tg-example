...
import (
	"strings"
	"sync"
	"tg-example/internal/transport/context"
...
func (http *httpExampleService) serveGet(ctx *fiber.Ctx) (err error) {
	return http._serveMethod(ctx, "get", http.get)
}
func (http *httpExampleService) get(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {

	var err error
	var request requestExampleServiceGet
	var response responseExampleServiceGet

	methodCtx := ctx.UserContext()
	methodCtx = log.Ctx(methodCtx).With().Str("method", "exampleService.get").Logger().WithContext(methodCtx)
	...
	response.Resp, err = http.svc.Get(methodCtx, request.Req)
	if err != nil {
...
func (http *httpExampleService) _serveMethod(ctx *fiber.Ctx, methodName string, methodHandler methodJsonRPC) (err error) {
...
func (http *httpExampleService) doBatch(ctx *fiber.Ctx, requests []baseJsonRPC) (responses jsonrpcResponses) {
...
func (http *httpExampleService) serveBatch(ctx *fiber.Ctx) (err error) {
...
func (http *httpExampleService) doSingleBatch(ctx *fiber.Ctx, request baseJsonRPC) (response *baseJsonRPC) {

	methodNameOrigin := request.Method
	method := strings.ToLower(request.Method)
	switch method {
	case "get":
		return http.get(ctx, request)
	default:
