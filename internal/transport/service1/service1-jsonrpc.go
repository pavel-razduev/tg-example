...
import (
	...
	"strings"
	"sync"
	"tg-example/internal/transport/service1/context"

	"github.com/gofiber/fiber/v2"
	"github.com/rs/zerolog/log"
)

func (http *httpService1) serveGet(ctx *fiber.Ctx) (err error) {
	return http._serveMethod(ctx, "get", http.get)
}
func (http *httpService1) get(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {

	var err error
	var request requestService1Get
	var response responseService1Get

	methodCtx := ctx.UserContext()
	methodCtx = log.Ctx(methodCtx).With().Str("method", "service1.get").Logger().WithContext(methodCtx)
...
func (http *httpService1) _serveMethod(ctx *fiber.Ctx, methodName string, methodHandler methodJsonRPC) (err error) {
...
func (http *httpService1) doBatch(ctx *fiber.Ctx, requests []baseJsonRPC) (responses jsonrpcResponses) {
...
func (http *httpService1) serveBatch(ctx *fiber.Ctx) (err error) {
...
func (http *httpService1) doSingleBatch(ctx *fiber.Ctx, request baseJsonRPC) (response *baseJsonRPC) {
