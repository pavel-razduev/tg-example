...
import (
	...
	"strings"
	"sync"
	"tg-example/internal/transport/service2/context"

	"github.com/gofiber/fiber/v2"
	"github.com/rs/zerolog/log"
)

func (http *httpService2) serveSet(ctx *fiber.Ctx) (err error) {
	return http._serveMethod(ctx, "set", http.set)
}
func (http *httpService2) set(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {

	var err error
	var request requestService2Set
	var response responseService2Set

	methodCtx := ctx.UserContext()
	methodCtx = log.Ctx(methodCtx).With().Str("method", "service2.set").Logger().WithContext(methodCtx)
	...
	response.Resp, err = http.svc.Set(methodCtx, request.Req)
	if err != nil {
...
func (http *httpService2) _serveMethod(ctx *fiber.Ctx, methodName string, methodHandler methodJsonRPC) (err error) {
...
func (http *httpService2) doBatch(ctx *fiber.Ctx, requests []baseJsonRPC) (responses jsonrpcResponses) {
...
func (http *httpService2) serveBatch(ctx *fiber.Ctx) (err error) {
...
func (http *httpService2) doSingleBatch(ctx *fiber.Ctx, request baseJsonRPC) (response *baseJsonRPC) {

	methodNameOrigin := request.Method
	method := strings.ToLower(request.Method)
	switch method {
	case "set":
		return http.set(ctx, request)
	default:
